...
class Opal_WP_Template extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            'opal-wp_template',
            esc_html__( 'Opal Elementor Template', 'auros-core' ),
            [
                'description' => esc_html__( 'Embed your saved elements.', 'auros-core' ),
            ]
        );
    }
    ...
    public function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title', 'auros-core' ); ?>:</label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>">
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'template_id' ) ); ?>"><?php esc_attr_e( 'Choose Template', 'auros-core' ); ?>:</label>
            <select class="widefat elementor-widget-template-select" id="<?php echo esc_attr( $this->get_field_id( 'template_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'template_id' ) ); ?>">
                <option value="">— <?php _e( 'Select', 'auros-core' ); ?> —</option>
                <?php
                foreach ( $templates as $template ) :
            ...
            <a target="_blank" class="elementor-edit-template"<?php echo $style; ?> href="<?php echo add_query_arg( 'elementor', '', get_permalink( $instance['template_id'] ) ); ?>">
                <i class="fa fa-pencil"></i> <?php echo __( 'Edit Template', 'auros-core' ); ?>
            </a>
        </p>
        <?php
    }
