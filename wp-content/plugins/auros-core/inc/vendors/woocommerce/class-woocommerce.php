...
if ( ! class_exists( 'osf_WooCommerce' ) ) :
	...
	class osf_WooCommerce {
		...
		public function mobile_handheld_footer_bar() {
			$links = array(
				'my-account' => '<a class="my-accrount-footer" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'auros-core' ) . '</a>',
                'search'     => '<a class="search-footer" href="">' . esc_attr__( 'Search', 'auros-core' ) . '</a><div class="site-search">' . get_search_form( false ) . '</div>',
				'cart'       => ' <a class="footer-cart-contents" href="' . esc_url( wc_get_cart_url() ) . '" 
                title="' . esc_attr__( 'View your shopping cart', 'auros-core' ) . '">  
 <span class="count">' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</span>
		...
		public function label_tooltip() {
			...
			echo '<div id="osf-woocommerce-cart">' . esc_html__( 'Add to cart', 'auros-core' ) . '</div>';
			echo '</div>';
		}
		...
		public function added_to_cart_template() {
			$text = esc_html__( 'has been added to your cart', 'auros-core' );
			echo <<<HTML
...
HTML;
		...
		public function widgets_init() {
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop', 'auros-core' ),
				'id'            => 'sidebar-woocommerce-shop',
				'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'auros-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Detail', 'auros-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop Filters', 'auros-core' ),
		...
		public function render_woocommerce_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<div id="opal-canvas-filter" class="opal-canvas-filter ' . $position . '"><span class="filter-close">' . esc_html__( 'CLOSE', 'auros-core' ) . '</span><div class="opal-canvas-filter-wrap">';
		...
		public function render_button_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__( 'Filter', 'auros-core' ) . '</button>';
			}
		}
			...
			if ( ! empty( $query ) ) {
				...
				if ( is_object( $product_object ) ) {
					...
					if ( ! empty( $product_sku ) ) {
						$product_sku_display = ' - ' . __( 'Sku', 'auros-core' ) . ': ' . $product_sku;
					}

					$product_title_display = '';
					if ( ! empty( $product_title ) ) {
						$product_title_display = ' - ' . __( 'Title', 'auros-core' ) . ': ' . $product_title;
					}

					$product_id_display = __( 'Id', 'auros-core' ) . ': ' . $product_id;
		...
		public function productCategoryAutocompleteRender( $query ) {
			...
			$term_slug_display = '';
			if ( ! empty( $term_slug ) ) {
				$term_slug_display = ' - ' . __( 'Sku', 'auros-core' ) . ': ' . $term_slug;
			}

			$term_title_display = '';
			if ( ! empty( $term_title ) ) {
				$term_title_display = ' - ' . __( 'Title', 'auros-core' ) . ': ' . $term_title;
			}

			$term_id_display = __( 'Id', 'auros-core' ) . ': ' . $term_id;
		...
		public function taxonomy_metaboxes() {
			...
			$cmb_term = new_cmb2_box( array(
				'id'           => 'product_cat',
				'title'        => __( 'Product Metabox', 'auros-core' ), // Doesn't output for term boxes
		...
		public function product_video_custom_field() {
			...
			$cmb    = new_cmb2_box( array(
				...
				'title'        => esc_html__( 'Product Video Config', 'auros-core' ),
			...
			$cmb->add_field( array(
				'name' => __( 'Product video', 'auros-core' ),
				'desc' => __( 'Supports video from youtube and vimeo.', 'auros-core' ),
				'id'   => $prefix . 'video',
				'type' => 'oembed',
			) );

			$cmb->add_field( array(
				'name'         => __( 'Video Thumbnail', 'auros-core' ),
		...
		public function set_sidebar( $name ) {
			if ( osf_is_product_archive() && is_active_sidebar('sidebar-woocommerce-shop') ) {
		...
		public function woocommerce_scripts() {
			...
			wp_enqueue_script( 'osf-woocommerce-main', trailingslashit( AUROS_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/main.js', array(
				'jquery',
				'wp-util'
			), false, true );

			if ( is_product() ) {
				wp_enqueue_script( 'osf-woocommerce-single', trailingslashit( AUROS_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/single.js', array(), false, true );
				if ( get_theme_mod( 'osf_woocommerce_single_product_tab_style', 'tab' ) == 'accordion' ) {
					wp_enqueue_script( 'osf-woocommerce-single-accordion', trailingslashit( AUROS_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true );
				}
			}
        ...
        public function custom_comment_form($comment_form) {
            ...
            if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
                $comment_form['comment_field'] .= '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'auros-core' ) . '</label><select name="rating" id="rating" required>
							<option value="">' . esc_html__( 'Rate&hellip;', 'auros-core' ) . '</option>
							<option value="5">' . esc_html__( 'Perfect', 'auros-core' ) . '</option>
							<option value="4">' . esc_html__( 'Good', 'auros-core' ) . '</option>
							<option value="3">' . esc_html__( 'Average', 'auros-core' ) . '</option>
							<option value="2">' . esc_html__( 'Not that bad', 'auros-core' ) . '</option>
							<option value="1">' . esc_html__( 'Very poor', 'auros-core' ) . '</option>
						</select></div>';
            }

            $comment_form['fields'] = array(
                'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__('Name', 'auros-core') . '&nbsp;<span class="required">*</span></label> ' .
                    '<input id="author" name="author" type="text" placeholder="' . esc_attr__( "Name", "auros-core" ) . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required /></p>',
                'email'  => '<p class="comment-form-email"><label for="email">' . esc_html__('Email', 'auros-core') . '&nbsp;<span class="required">*</span></label> ' .
                    '<input id="email" name="email" type="email" placeholder="' . esc_attr__( "Email", "auros-core" ) . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" required /></p>',
            );
            $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'auros-core' ) . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required placeholder="' . esc_attr__( "Your review", "auros-core" ) . '"></textarea></p>';
            return $comment_form;
        }
