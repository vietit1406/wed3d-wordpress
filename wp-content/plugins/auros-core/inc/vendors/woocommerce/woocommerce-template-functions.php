...
if (!function_exists('osf_cart_link')) {
    ...
    function osf_cart_link()
    {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            $items = '';
            $items .= '<a data-toggle="toggle" class="cart-contents header-button" href="' . esc_url(wc_get_cart_url()) . '" title="' . __("View your shopping cart", "auros-core") . '">';
...
if (!function_exists('osf_cart_count_text')) {
    ...
    {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            return '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "auros-core")) . '</span>';
        }
...
if (!function_exists('osf_handheld_footer_bar_search')) {
    ...
    {
        echo '<a href="">' . esc_attr__('Search', 'auros-core') . '</a>';
        osf_product_search();
    }
}
...
if (!function_exists('osf_handheld_footer_bar_cart_link')) {
    ...
    function osf_handheld_footer_bar_cart_link()
    {
        ...
        <a class="footer-cart-contents" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'auros-core'); ?>">
...
if (!function_exists('osf_handheld_footer_bar_account_link')) {
    ...
    {
        echo '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'auros-core') . '</a>';
    }
}
...
if (!function_exists('osf_woocommerce_order_review_heading')) {
    ...
    {
        echo ' <h3 class="h2 order_review_heading">'. esc_attr__('Your order', 'auros-core') .'</h3>';
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_stock')) {
    function osf_woocommerce_get_product_label_stock()
    {
        ...
        if ($product->get_stock_status() == 'outofstock') {
            echo '<span class="out-of-stock">' . esc_html__('Out Of Stock', 'auros-core') . '</span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_set_register_text')) {
    ...
    {
        echo '<div class="user-text">' . __("Creating an account is quick and easy, and will allow you to move through our checkout quicker.", "auros-core") . '</div>';
    }
}
        ...
        if ($time_sale) {
            ...
            echo '<div class="time">
                    <div class="opal-countdown clearfix"
                        ...
                        data-days="' . esc_html__("days", "auros-core") . '" 
                        data-hours="' . esc_html__("hours", "auros-core") . '"
                        data-minutes="' . esc_html__("mins", "auros-core") . '"
                        data-seconds="' . esc_html__("secs", "auros-core") . '"
                        data-Message="' . esc_html__('Expired', 'auros-core') . '"
...
if (!function_exists('osf_active_filters')) {
    function osf_active_filters()
    {
        ...
        echo '<span class="osf_active_filters_label">' . esc_html__('Active Filters: ', 'auros-core') . '</span>
    <a class="clear-all" href="' . esc_url($link_remove_all) . '">' . __('Clear Filters', 'auros-core') . '</a>
</div>';
    }
}
...
function osf_single_product_social()
{
    if (get_theme_mod('osf_socials')) {
        $template = AUROS_CORE_PLUGIN_DIR . 'templates/socials.php';
...
if (!function_exists('osf_woocommerce_deal_progress')) {
    function osf_woocommerce_deal_progress()
    {
        ...
        <div class="deal-sold">
            <span class="deal-text d-block"><span><?php esc_html_e('HURRY! ONLY', 'auros-core') ?></span> <span
                        class="c-primary"><?php echo esc_attr(trim($limit - $sold)) ?></span> <span><?php esc_html_e('LEFT IN STOCK.', 'auros-core') ?></span></span>
            <div>
...
function otf_woocommerce_recently_viewed_product()
{
    if (get_theme_mod('otf_woocommerce_extra_enable_product_recently_viewed', false)) {
        ...
        if (!empty($_COOKIE['otf_woocommerce_recently_viewed'])) {
            ...
            echo '<h2 class="otf-woocommerce-recently-viewed">' . esc_html__('Your Recently Viewed', 'auros-core') . '</h2>';
